...
def search_cpvs(entry_component=None):
    matching_cpvs = []
...
def get_available_options():
    ...
    return sorted(entry_components)


def search_cpvs_with_suggestions(entry_component=None):
    matching_cpvs = search_cpvs(entry_component)
    
    if not matching_cpvs:
        available_components = get_available_options()
        
        if entry_component:
            comp_suggestions = get_suggestions(entry_component, available_components)
            if comp_suggestions:
                print("No exact matches found. Did you mean:")
                print(f"  Entry component suggestions: {', '.join(comp_suggestions)}")
                print()
...
if __name__ == "__main__":
    print("=== CPV Search Tool (By Component) ===")
    ...
    while True:
        ...
        results = search_cpvs_with_suggestions(entry_component)

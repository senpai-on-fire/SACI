...
def search_cpvs(entry_component=None, attack_vector=None, attack_impact=None):
    ...
    for cpv in CPVS:
        ...
        if attack_vector:
            vector_match = any(
                attack_vector.lower() in vec.name.lower() for vec in cpv.attack_vectors
            )     
            if not vector_match:
                continue
        ...
        if attack_impact:
            impact_match = any(
                attack_impact.lower() in impact.category.lower() for impact in cpv.attack_impacts
            )
            if not impact_match:
                continue
...
def format_results(cpvs):
    formatted_results = []
    for cpv in cpvs:
        initial_conditions_formatted = "\n        - " + "\n        - ".join([f"{key}: {value}" for key, value in cpv.initial_conditions.items()])
        ...
        Attack Requirements:
        - """ + "\n        - ".join(cpv.attack_requirements) + f"""
        Attack Vectors:
        - """ + "\n        - ".join([vec.name for vec in cpv.attack_vectors]) + f"""
        Attack Impacts:
        - """ + "\n        - ".join([impact.description for impact in cpv.attack_impacts]) + f"""
        Exploit Steps:
        - """ + "\n        - ".join(cpv.exploit_steps) + f"""
